...
static void example_do_connect(mqtt_client_t *client)
{
  ...
  ip_addr_t mqqt_server_addr;
  IP4_ADDR( &mqqt_server_addr, 5, 196, 95, 208 );
  err = mqtt_client_connect(client, &mqqt_server_addr, MQTT_PORT, mqtt_connection_cb, 0, &ci);
...
void StartDefaultTask(void const * argument)
{
  ...
  for(;;)
  {
	...
	if (b!=old) {
	  if (b) {
		if ( mqtt_client_is_connected(&static_client) ) {
		  example_publish(&static_client, "LRED=1", 0);
		  osDelay(50); // to prevent flooding of packets in the TCP/IP stack buffer
		}
	  } else {
		if ( mqtt_client_is_connected(&static_client) ) {
		  example_publish(&static_client, "LRED=0", 0);
		  osDelay(50); // to prevent flooding of packets in the TCP/IP stack buffer
		}
	  }
	  old=b;
